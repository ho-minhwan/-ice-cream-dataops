...
def get_time_series_for_site(client: CogniteClient, site):
    ...
    print(f"[DEBUG] site={site!r}")
    print(f"[DEBUG] expected node ID=icapi_dm_space:{this_site}")
    ...
    print(f"[DEBUG] sub_tree_root={sub_tree_root!r}")
    ...
    #     print(
    #         f"----CogniteAsset root exists, but path is empty for {site}!----\n"
    #         f"    Node: {sub_tree_root.space}:{sub_tree_root.external_id}\n"
    #         f"    Check the create_asset_hierarchy transformation output."
    #     )
    #     print(f"[DEBUG] root space={sub_tree_root.space!r}")
    #     print(f"[DEBUG] root external_id={sub_tree_root.external_id!r}")
    #     print(f"[DEBUG] root path={sub_tree_root.path!r}")
    #     return []
    if not sub_tree_root.path:
        print(
            f"[INFO] Root path missing for {site}, "
            "continuing with direct asset search."
        )
...
def handle(client: CogniteClient = None, data=None):
    ...
    try:
        for site in sites:
            ...
            if not time_series:
                print(f"[INFO] Skipping {site}: no matching TimeSeries found.")
                continue
